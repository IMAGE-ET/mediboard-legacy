...
function checkPlage() {
  ...
  if (field = form.chir_id)
    if (field.value == -1) {
      alert("Merci de choisir un anesthésiste");
      field.focus();
      return false;
    }
  ...
  <tr>
    <td>
      <table width="100%">
              ...
              {if $curr_plage->_hour_deb == $curr_hour}
                <td align="center" bgcolor="#aaaaaa" rowspan="{$curr_plage->_hour_fin-$curr_plage->_hour_deb}">
                  <a href="index.php?m={$m}&amp;tab={$tab}&amp;plageconsult_id={$curr_plage->plageconsult_id}">
                    ...
                    {$curr_plage->_ref_consultations_anesth|@count} consult(s)
                  </a>
                </td>
              {/if}
        ...
        <tr>
          <td colspan="8">
            <form name='editFrm' action='?m=dPanesth' method='post' onsubmit='return checkPlage()'>
        ...
        <tr>
          <td colspan="8">
          <form name='removeFrm' action='./index.php?m=dPanesth' method='post'>
          <input type='hidden' name='dosql' value='do_plageconsult_aed' />
      ...
      <table class="tbl">
        ...
        <tr>
          ...
          <td {$style}>
            ...
            <form name="cancelFrm{$curr_consult->consultation_anesth_id}" action="?m={$m}" method="POST">
            <input type="hidden" name="m" value="{$m}" />
            <input type="hidden" name="dosql" value="do_consultation_aed" />
            <input type="hidden" name="consultation_anesth_id" value="{$curr_consult->consultation_anesth_id}" />
            <input type="hidden" name="_check_premiere" value="{$curr_consult->_check_premiere}" />
            ...
            <a class="action" href="javascript:document.etatFrm{$curr_consult->consultation_anesth_id}.submit()">
              <img src="modules/{$m}/images/check.png" title="Notifier l'arrivée du patient">
            </a>
            <a class="action" href="javascript:document.cancelFrm{$curr_consult->consultation_anesth_id}.submit()">
